...
const renderPlugin = ViewPlugin.fromClass(
  class {
    ...
    update(update: ViewUpdate) {
      // Rebuild decorations if doc changed, cursor has moved, or suggestion changed
    ...
    build(view: EditorView) {
      ...
      return Decoration.set([
        Decoration.widget({
          ...
          side: 1, // Redner after cursor (side: 1), not before (side: -1)
        }).range(cursor),
      ]);
    }
  },
...
const acceptSuggestionKeymap = keymap.of([
  {
    ...
    run: (view) => {
      const suggestion = view.state.field(suggestionState);
      if (!suggestion) {
        return false; // No suggestion found. Let Tab do its normal thing (indent)
      }
